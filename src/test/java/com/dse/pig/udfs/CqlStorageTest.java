...
public class CqlStorageTest {
	...
	public void testParseUUID() throws IOException {
		pigContext.connect();
//		pig.registerQuery("a = LOAD '"+ INPUT_FILE + "' USING com.dse.pig.udfs.CqlStorage AS (bucket:chararray, timeuuid:chararray, uuid:chararray, events:chararray);");
//		pig.registerQuery("a = LOAD 'cql://applications/events?init_address=192.168.3.100' USING org.apache.cassandra.hadoop.pig.CqlStorage() AS (bucket:chararray, timeuuid:chararray, uuid:chararray, events:chararray);");
		pig.registerQuery("a = LOAD 'cql://applications/events?init_address=192.168.3.100' USING com.dse.pig.udfs.CqlStorage() AS (bucket:chararray, timeuuid:chararray, uuid:chararray, events:chararray);");
...
//		pig.registerQuery("b = FOREACH a GENERATE (chararray)uuid;");
		...
		Iterator<Tuple> iterator = pig.openIterator("a");
